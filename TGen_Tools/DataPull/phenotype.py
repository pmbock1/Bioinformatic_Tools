...
# USAGE: Used for large scale retrieval of phenotype calls of PharmCat JSON output
# Useful for cohort analysis of metabolism levels
# How to use: In command line : python3 ./phenotype.py /path/to/Directory_to_Search /path/to/filetowrite.csv

def genepull(direc, cfile):
                            ...
                            if keys == "genotypes":  # only considers genotype field
                                for genes in values:
                                    for subfields, genotypes in genes.items():
                                        if subfields == "phenotype":  # Final dictionary that contains genotype of each gene
                                            if "," in str(genotypes):
...
if __name__ == "__main__":
    ...
    genepull(sys.argv[1], sys.argv[2])
